...
const CampaignList = () => {
	...
	const handleDonate = async (campaignId, amount, currency) => {
		try {
			console.log(campaignId, amount, currency);
			...
			// Fetch conversion rates if the currency is not ETH
			if (currency !== "ETH") {
				const apiEndpoint = `https://api.coinbase.com/v2/prices/${currency}-ETH/spot`;
				try {
					...
					toast.error(`Error fetching conversion rate for ${currency}`);
					return;
				}
			}
			...
			const decimalPlaces = ethAmount.toString().split(".")[1]?.length || 0;
			if (decimalPlaces > 18) {
				toast.error("Amount must be greater than 0.000000000000000001 ETH");
				return;
			}
			...
			// Proceed with blockchain donation using ETH
											...
											<div className="flex flex-col gap-4">
												<div className="flex gap-2">
													<input
														...
														value={donationAmounts[campaign.id]?.amount || ""}
														onChange={(e) =>
													...
													{/* Currency Selector */}
													<select
														className="px-4 py-3 bg-gray-800/30 border border-gray-700/50 text-gray-100 rounded-xl focus:ring-2 focus:ring-indigo-500/50 focus:outline-none transition-all"
														value={
															donationAmounts[campaign.id]?.currency || "ETH"
														}
														onChange={(e) =>
															setDonationAmounts((prev) => ({
																...prev,
													...
													>
														<option value="ETH">Donate in ETH</option>
														<option value="USD">Donate in USD</option>
														<option value="BDT">Donate in BDT</option>
													</select>
												</div>
												...
												<button
													onClick={() =>
														handleDonate(
															campaign.id,
															donationAmounts[campaign.id]?.amount,
															donationAmounts[campaign.id]?.currency
														)
													}
